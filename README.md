...
**Phases 1, 2 and 3 complete.** The decision engine is written and green
against the golden corpus, it can be driven from a YAML file, and it can now
carry out what it decides — encode to a temp file, verify the output, and
...
`go test ./...` runs 207 tests. The headline one is `TestGoldenCorpus`, which
...
The encoder's tests all work on clips generated by ffmpeg — two seconds of
`testsrc` and a sine wave — never on anything from a real library. Each safety
rule has a test that proves the original file survives when the rule fires.
...
Still to build: the background scanner and queue, the web UI, and the
container. See [docs/plan.md](docs/plan.md).
